...
# Test ProfilePictures Collection - Phase 2.2 Enhanced
# Tests the new ProfilePictures collection with proper file storage

echo "🧪 Testing ProfilePictures Collection (V2)"
echo "========================================"
...
echo -e "\n${YELLOW}Step 2: Test Profile Picture Upload (V2)${NC}"

# Create a test image file (1KB PNG)
echo "Creating test image file..."
cat > test_profile.png << 'EOF'
iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==
EOF

# Convert base64 to binary
echo "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==" | base64 -d > test_profile_binary.png

echo -e "\n${BLUE}Testing: Upload Profile Picture V2${NC}"
UPLOAD_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/api/advertiser-dashboard?action=upload-profile-picture-v2" \
  -H "Authorization: Bearer $TOKEN" \
  -F "file=@test_profile_binary.png")

UPLOAD_STATUS=$(get_status_code "$UPLOAD_RESPONSE")
UPLOAD_BODY=$(echo "$UPLOAD_RESPONSE" | sed '$d')

if [ "$UPLOAD_STATUS" = "200" ]; then
    echo -e "${GREEN}✅ Profile picture upload successful${NC}"
    PROFILE_PICTURE_ID=$(echo "$UPLOAD_BODY" | jq -r '.data.id')
    PROFILE_PICTURE_URL=$(echo "$UPLOAD_BODY" | jq -r '.data.url')
...
echo -e "\n${YELLOW}Step 3: Test Get Profile Picture${NC}"

GET_RESPONSE=$(curl -s -w "\n%{http_code}" -X GET "$BASE_URL/api/advertiser-dashboard?action=get-profile-picture" \
  -H "Authorization: Bearer $TOKEN")
...
if [ "$GET_STATUS" = "200" ]; then
    echo -e "${GREEN}✅ Get profile picture successful${NC}"
    echo "Response: $GET_BODY"
    ((TESTS_PASSED++))
else
    echo -e "${RED}❌ Get profile picture failed${NC}"
    echo "Response: $GET_BODY"
    ((TESTS_FAILED++))
fi

echo -e "\n${YELLOW}Step 4: Test File Validation${NC}"

# Test invalid file type
echo -e "\n${BLUE}Testing: Invalid file type (.exe)${NC}"
echo "test content" > test_file.exe
...
echo -e "\n${YELLOW}Step 5: Test Profile Picture Replacement${NC}"

# Create another test image
cat > test_profile2.png << 'EOF'
iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==
EOF
echo "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==" | base64 -d > test_profile2_binary.png

REPLACE_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/api/advertiser-dashboard?action=upload-profile-picture-v2" \
  -H "Authorization: Bearer $TOKEN" \
  -F "file=@test_profile2_binary.png")

REPLACE_STATUS=$(get_status_code "$REPLACE_RESPONSE")
REPLACE_BODY=$(echo "$REPLACE_RESPONSE" | sed '$d')

if [ "$REPLACE_STATUS" = "200" ]; then
    echo -e "${GREEN}✅ Profile picture replacement successful${NC}"
    NEW_PROFILE_PICTURE_ID=$(echo "$REPLACE_BODY" | jq -r '.data.id')
    echo "New Profile Picture ID: $NEW_PROFILE_PICTURE_ID"
    ((TESTS_PASSED++))
else
    echo -e "${RED}❌ Profile picture replacement failed${NC}"
    echo "Response: $REPLACE_BODY"
    ((TESTS_FAILED++))
fi

echo -e "\n${YELLOW}Step 6: Test Delete Profile Picture${NC}"

if [ ! -z "$PROFILE_PICTURE_ID" ]; then
    DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/api/advertiser-dashboard?action=delete-profile-picture" \
      -H "Authorization: Bearer $TOKEN" \
      -H "Content-Type: application/json" \
    ...
    if [ "$DELETE_STATUS" = "200" ]; then
        echo -e "${GREEN}✅ Delete profile picture successful${NC}"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}❌ Delete profile picture failed${NC}"
...
echo -e "\n${YELLOW}Step 7: Test Business Profile Update${NC}"

# Check if business profile was updated with profile picture info
...
if [ "$BUSINESS_STATUS" = "200" ]; then
    ...
    if [ "$PROFILE_PICTURE_URL" != "null" ] && [ "$PROFILE_PICTURE_URL" != "" ]; then
        echo -e "${GREEN}✅ Business profile updated with profile picture${NC}"
        echo "Profile Picture URL: $PROFILE_PICTURE_URL"
        ((TESTS_PASSED++))
    else
        echo -e "${RED}❌ Business profile not updated with profile picture${NC}"
        ((TESTS_FAILED++))
    fi
else
...
rm -f test_profile.png test_profile_binary.png test_profile2.png test_profile2_binary.png test_file.exe large_file.png
...
if [ $TESTS_FAILED -eq 0 ]; then
    echo -e "\n${GREEN}🎉 All tests passed! ProfilePictures collection is working correctly.${NC}"
    exit 0
else
