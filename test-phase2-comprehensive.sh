...
# Test credentials (you can update these)
...
if [ -z "$TOKEN" ]; then
  echo "❌ Partner login failed. Trying regular user login..."
  
  # Try regular user login as fallback
  USER_LOGIN=$(curl -s -X POST "$SERVER/api/users/login" \
    -H "Content-Type: application/json" \
    -d '{"email": "feniola07@gmail.com", "password": "Test@2025"}')
  
  echo "User Login Response: $USER_LOGIN"
  TOKEN=$(echo $USER_LOGIN | grep -o '"token":"[^"]*"' | cut -d'"' -f4)
...
echo ""
echo "📊 Step 2: Testing Dashboard Overview"
echo "------------------------------------"
...
echo ""
echo "👤 Step 3: Testing Profile Management"
echo "------------------------------------"
...
# Update profile (if business exists)
...
echo ""
echo "🎯 Step 4: Testing Campaign Management"
echo "------------------------------------"
...
if [ -n "$CAMPAIGN_ID" ]; then
  ...
  echo ""
  echo "📁 Step 5: Testing Media Upload"
  echo "------------------------------"
  ...
  echo ""
  echo "📈 Step 6: Testing Performance Tracking"
  echo "--------------------------------------"
  ...
  echo ""
  echo "🧾 Step 7: Testing Invoice Generation"
  echo "------------------------------------"
  ...
  echo "❌ Failed to create campaign - no business profile found"
fi
